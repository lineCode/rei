...
void reiProgramInitialize(ReiContext * context, ReiProgramBinding binding, unsigned codeStringLengthWithoutNullTerminator, char * codeString) {
  const ReiTypeContext * ctx = (ReiTypeContext *)(void *)context;
...
void reiTextureInitialize(ReiContext * context, ReiTextureBinding binding, ReiSamplerFiltering magFiltering, ReiSamplerFiltering minFiltering, ReiSamplerBehaviorOutsideTextureCoordinate behaviorOutsideTextureCoordinateU, ReiSamplerBehaviorOutsideTextureCoordinate behaviorOutsideTextureCoordinateV, int maxAnisotropy, ReiBool32 generateMipLevels) {
  const ReiTypeContext * ctx = (ReiTypeContext *)(void *)context;
...
void reiTextureDefineAndUploadTexels(ReiContext * context, ReiTextureBinding binding, int bindingLevel, ReiTextureTexelFormat bindingTexelFormat, int width, int height, ReiTextureTexelFormat texelsFormat, ReiTextureTexelType texelsType, int texelsBytesAlignment, void * texels) {
  const ReiTypeContext * ctx = (ReiTypeContext *)(void *)context;
...
void reiTextureUploadTexels(ReiContext * context, ReiTextureBinding binding, int bindingLevel, int bindingX, int bindingY, int width, int height, ReiTextureTexelFormat texelsFormat, ReiTextureTexelType texelsType, int texelsBytesAlignment, void * texels) {
  const ReiTypeContext * ctx = (ReiTypeContext *)(void *)context;
  ctx->glPixelStorei(0x0CF5, texelsBytesAlignment); // GL_UNPACK_ALIGNMENT
  ctx->glTexSubImage2D(binding, bindingLevel, bindingX, bindingY, width, height, texelsFormat, texelsType, texels);
}

void reiTextureDefineAndCopyTexelsFromBackbuffer(ReiContext * context, ReiTextureBinding binding, int bindingLevel, ReiTextureTexelFormat bindingTexelFormat, int backbufferX, int backbufferY, int width, int height) {
  const ReiTypeContext * ctx = (ReiTypeContext *)(void *)context;
  ctx->glCopyTexImage2D(binding, bindingLevel, bindingTexelFormat, backbufferX, backbufferY, width, height, 0);
}

void reiTextureCopyTexelsFromBackbuffer(ReiContext * context, ReiTextureBinding binding, int bindingLevel, int bindingX, int bindingY, int backbufferX, int backbufferY, int width, int height) {
  const ReiTypeContext * ctx = (ReiTypeContext *)(void *)context;
  ctx->glCopyTexSubImage2D(binding, bindingLevel, bindingX, bindingY, backbufferX, backbufferY, width, height);
}

void reiReadbackTexelsFromBackbuffer(ReiContext * context, int backbufferX, int backbufferY, int width, int height, ReiTextureTexelFormat texelsFormat, ReiTextureTexelType texelsType, int texelsBytesAlignment, void * outTexels) {
  const ReiTypeContext * ctx = (ReiTypeContext *)(void *)context;
...
void reiCommandClear(ReiContext * context, ReiClearFlags clear, float depthValue, int stencilValue, float colorR, float colorG, float colorB, float colorA) {
  const ReiTypeContext * ctx = (ReiTypeContext *)(void *)context;
...
void reiCommandMeshSetState(ReiContext * context, ReiMeshState * state) {
  ...
  ctx->glViewport(state->viewportX, state->viewportY, state->viewportWidth, state->viewportHeight);
  ctx->glScissor(state->scissorX, state->scissorY, state->scissorWidth, state->scissorHeight);
...
char * reiGetProgramStatusString(ReiContext * context) {
  const ReiTypeContext * ctx = (ReiTypeContext *)(void *)context;
