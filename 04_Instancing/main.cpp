...
int main() {
  ...
  char vp_string[] =
    ...
    "ADD pos.y, R0.x,-R0.y;"
    "ADD pos.z, R0.z, R0.w;"
    "MUL R0, vertex.color, program.local[2];"
    "MOV result.color, R0;"
  ...
  reiCommandListSet(ctx, list_mesh_state);
    ...
    mesh_state.viewportWidth                                  = 1000;
    mesh_state.viewportHeight                                 = 1000;
    mesh_state.scissorX                                       = 0;
    mesh_state.scissorY                                       = 0;
    mesh_state.scissorWidth                                   = 1000;
    mesh_state.scissorHeight                                  = 1000;
  ...
  reiCommandListSet(ctx, list_mesh);
    ...
    for (int i = 0, mesh_vertices_count = countof(mesh_vertices) / 3; i < mesh_vertices_count; i += 1) {
      reiCommandMeshColor(ctx, i * 0.00075f, i * 0.00045f, i * 0.00065f, 1);
  ...
  for (float pos_x = -25, pos_y = -25, pos_z = -75, rot_x = 0, rot_y = 0; glfwWindowShouldClose(window) == 0; glfwPollEvents()) {
    double mouse_x = 0;
    double mouse_y = 0;
